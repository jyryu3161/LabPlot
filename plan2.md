...
| (완료) | "반영 안 됨"의 근본 = AI 패치 어휘(스키마)가 흔한 편집 의도를 커버 못하면 무성 드롭. 사용자 구체 예시 대신 **33개 흔한 편집 의도 → 옵션 키 커버리지 회귀**(ai-edit-quality.spec.js)로 고정 — 배포 스키마가 그 키들을 전부 커버함을 결정론적으로 검증(키 누락 시 실패). 라이브 시연: "x축 라벨 45도 회전+레전드 하단" → x_text_angle=45·legend_position=bottom 정확 반영, satisfied=true |
