...
// Public API calls are same-origin in production. Local UI regression runs can
// target the separately exposed backend without adding a Next.js-only proxy.
const E2E_API_BASE = (process.env.E2E_API_BASE ?? '').replace(/\/$/, '');
...
test('public gallery exposes template actions', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: 'View template details' }).first()).toBeVisible();

  const gallery = await page.evaluate(async (apiBase) => {
    const res = await fetch(`${apiBase}/api/public/gallery?limit=1`);
    if (!res.ok) throw new Error(await res.text());
    return res.json() as Promise<{ figures: { id: string }[] }>;
  }, E2E_API_BASE);
  expect(gallery.figures.length).toBeGreaterThan(0);

  const template = await page.evaluate(async ({ id, apiBase }) => {
    const res = await fetch(`${apiBase}/api/public/gallery/${id}/template`);
    if (!res.ok) throw new Error(await res.text());
    return res.json() as Promise<{ example_data?: { download_url: string; preview: unknown[] } }>;
  }, { id: gallery.figures[0].id, apiBase: E2E_API_BASE });
  expect(template.example_data?.preview.length).toBeGreaterThan(0);

  const downloadStatus = await page.evaluate(async ({ url, apiBase }) => {
    const resolved = url.startsWith('/') ? `${apiBase}${url}` : url;
    const res = await fetch(resolved);
    return { status: res.status, contentType: res.headers.get('content-type') };
  }, { url: template.example_data!.download_url, apiBase: E2E_API_BASE });
...
test('authenticated user can open the gallery template flow', async ({ page }) => {
  ...
  await page.goto('/gallery');
  await page.getByRole('link', { name: 'View template details' }).first().click();
...
test('authenticated user can create a figure from a gallery template with a complex Excel file', async ({ page }, testInfo) => {
  ...
  const project = await page.evaluate(async ({ projectName, headers, apiBase }) => {
  ...
  }, { projectName, headers, apiBase: E2E_API_BASE });

  try {
    const gallery = await page.evaluate(async (apiBase) => {
      const res = await fetch(`${apiBase}/api/public/gallery?limit=80`);
      if (!res.ok) throw new Error(await res.text());
    ...
    await page.evaluate(async ({ id, headers, apiBase }) => {
      await fetch(`${apiBase}/api/projects/${id}`, { method: 'DELETE', headers });
    }, { id: project.id, headers, apiBase: E2E_API_BASE });
  }
});
