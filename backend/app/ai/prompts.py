...
- A region selecting one renderer-approved grouped_bar bar, scatter point, heatmap cell, or correlation_heatmap cell may change only that element's fill or stroke through options.element_overrides, as an array entry {"id": "<exact resolved_target.element_id>", "fill": "#RRGGBB"} (or "stroke" instead of/with "fill"). Never invent or rewrite an element id. An element whose resolved_target.editable is false remains unsupported and must receive no fallback patch. A named categorical level may use options.category_colors only when the exact category label is stated in the user's memo.
- SCHEMA IS AUTHORITATIVE: every key offered in the param_patch options schema is supported by the renderer for THIS plot type. Never declare such a key unsupported on statistical or stylistic grounds (for example a log scale on a box plot, or a legend layout change): emit the patch and let the user decide. Reserve "unsupported" strictly for requests that no schema key can express.
...
- If the user asks to change one specific category/group color, set options.category_colors as an array of {"level": "<exact visible group label>", "color": "<HEX color>"} entries, for example [{"level": "Control", "color": "#4477AA"}]. Use this only for categorical fill/colour encodings.
...
- For one server-resolved editable bar/point/cell only, options.element_overrides may be [{"id": "<exact resolved_target.element_id>", "fill": "#RRGGBB"}] or the same entry shape with "stroke" instead of/with "fill". Do not include geometry, values, alpha, width, labels, mappings, or sibling ids.
