...
# ----------------------------------------------------------------- canvas style
def suggest_canvas_style(db: Session, canvas_context: dict, palettes: list[dict],
                         user_id: uuid.UUID | None = None) -> dict:
    palette_keys = [p.get("key") for p in palettes if p.get("key")]
    if not palette_keys:
        palette_keys = ["okabe_ito"]
    schema = {
        "type": "object",
        "properties": {
            "palette_key": {"type": "string", "enum": palette_keys},
            "font_size": {"type": "integer"},
            "layout": {"type": "string", "enum": ["keep", "grid", "two_column", "single_row"]},
            "rationale": {"type": "string"},
        },
        "required": ["palette_key", "font_size", "layout", "rationale"],
    }
    system = (
        "You are a publication multi-panel figure layout assistant. "
        "Choose one available color palette, one unified font size in points, and a conservative layout strategy. "
        "Only choose from the provided palette keys. Prefer colorblind-safe palettes for mixed scientific panels. "
        "Do not suggest changing data, labels, or chart content."
    )
    ctx = {
        "canvas": canvas_context,
        "available_palettes": palettes,
        "constraints": {
            "font_size_range": [6, 18],
            "layout_values": ["keep", "grid", "two_column", "single_row"],
        },
    }
    out = _run_logged(
        db,
        user_id,
        "canvas_style_suggestion",
        system,
        [{"kind": "text", "text": "Canvas style context:\n" + json.dumps(ctx, ensure_ascii=False)}],
        schema,
        "canvas_style_suggestion",
        900,
    )
    palette_key = out.get("palette_key") if out.get("palette_key") in palette_keys else palette_keys[0]
    try:
        font_size = int(out.get("font_size") or 9)
    except (TypeError, ValueError):
        font_size = 9
    font_size = max(6, min(18, font_size))
    layout = out.get("layout") if out.get("layout") in {"keep", "grid", "two_column", "single_row"} else "grid"
    return {
